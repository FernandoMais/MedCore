...
const PatientManager: React.FC<PatientManagerProps> = ({ patients, setPatients, doctors, isAdmin }) => {
  ...
  // Função robusta para renderizar histórico separado por datas
  const renderFormattedHistory = (historyText: string) => {
    ...
    // Regex que identifica o bloco [DATA: dd/mm/aaaa às hh:mm]
      ...
      <div className="space-y-4">
        {parts.map((part, index) => {
          if (part.startsWith('[DATA:')) {
            return (
              <div key={index} className="flex items-center space-x-3 pt-6 pb-2 border-b border-slate-100 mt-4">
          ...
          return (
            <div key={index} className="bg-white border border-slate-100 p-8 rounded-[32px] text-sm font-medium text-slate-700 leading-relaxed shadow-sm hover:shadow-md transition-shadow whitespace-pre-wrap">
              {part.trim()}
            </div>
          );
        })}
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500 h-full pb-10">
      {/* Aviso de Permissões PT-BR */}
      <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-center justify-between no-print mb-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 no-print">
        ...
        <div className="flex items-center space-x-3">
          <button 
            ...
            className="bg-emerald-600 text-white px-6 py-3.5 rounded-2xl font-black text-xs shadow-xl hover:bg-emerald-700 flex items-center space-x-2"
          >
            <ShieldCheck size={18} />
          ...
          <button 
            ...
            className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl font-black text-xs shadow-xl hover:bg-blue-700 flex items-center space-x-2 transition-all active:scale-95"
          >
            <Plus size={20} />
            ...
            <table className="w-full text-left">
              <tbody className="divide-y divide-slate-50">
                ...
                {filteredPatients.length === 0 && (
                  <tr>
                    <td className="px-6 py-10 text-center text-slate-400 font-bold uppercase text-[10px] tracking-widest">Nenhum paciente encontrado.</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div className="bg-white border border-slate-200 rounded-[40px] shadow-2xl overflow-hidden flex flex-col h-full">
              <div className="p-10 border-b border-slate-100 bg-slate-50/30">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
                  <div className="flex items-center space-x-6">
                    <div className={`w-24 h-24 rounded-[32px] flex items-center justify-center text-4xl font-black text-white shadow-xl ${selectedPatient.allergies.length > 0 ? 'bg-red-600' : 'bg-blue-600'}`}>
                      {selectedPatient.name.charAt(0)}
                    </div>
                    <div>
                      <h2 className="text-3xl font-black text-slate-900 tracking-tighter">{selectedPatient.name}</h2>
                      <p className="text-sm font-bold text-slate-500 mt-1 uppercase tracking-widest">NASC: {new Date(selectedPatient.birthDate).toLocaleDateString('pt-BR')} • CPF: {selectedPatient.cpf}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 no-print">
                    <button 
                      onClick={handlePrint}
                      className="flex items-center space-x-3 px-6 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-black shadow-xl"
                    >
                      <Printer size={18} />
                      <span>IMPRIMIR PRONTUÁRIO</span>
                    </button>
                    <button onClick={startEdit} className="p-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 text-blue-600 shadow-sm"><Edit3 size={20} /></button>
                    <button onClick={() => setSelectedPatientId(null)} className="p-4 bg-slate-100 text-slate-500 rounded-2xl hover:bg-slate-200"><X size={20} /></button>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 p-10 space-y-10 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="bg-red-50 p-8 rounded-[32px] border border-red-100 print-alert">
                    <h3 className="text-xs font-black text-red-600 uppercase tracking-widest mb-4 flex items-center">
                      <AlertTriangle size={16} className="mr-2" /> Alertas de Segurança & Alergias
                    </h3>
                    <div className="flex flex-wrap gap-2">
                      {selectedPatient.allergies.length > 0 ? (
                        selectedPatient.allergies.map(a => <span key={a} className="bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded-lg uppercase">{a}</span>)
                      ) : <p className="text-xs text-red-400 font-bold italic">Nenhuma alergia relatada pelo paciente.</p>}
                    </div>
                  </div>
                  <div className="bg-slate-50 p-8 rounded-[32px] border border-slate-100 print-alert">
                    <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest mb-4 flex items-center">
                      <History size={16} className="mr-2 text-blue-500" /> Condições Pré-existentes
                    </h3>
                    <div className="flex flex-wrap gap-2">
                      ...
                      ) : <p className="text-xs text-slate-400 font-bold italic">Sem registros prévios no sistema.</p>}
                    </div>
                  </div>
                </div>

                {/* Histórico Cronológico por Atendimento */}
                <div className="space-y-6">
                   <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center px-2">
                      <FileText size={18} className="mr-3 text-blue-600" /> Evoluções Clínicas Fracionadas por Data
                   </h3>
                ...
                <div className="space-y-6 no-print pt-8 border-t border-slate-100">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center">
                      <FileText size={18} className="mr-3 text-emerald-600" /> Central de Documentos & Laudos Digitais
                    </h3>
                    <label className="flex items-center space-x-2 bg-emerald-600 text-white px-6 py-3 rounded-2xl text-xs font-black cursor-pointer hover:bg-emerald-700 shadow-xl shadow-emerald-100 transition-all active:scale-95">
                      <Upload size={18} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(selectedPatient.files || []).length > 0 ? (
                        ...
                        <div key={file.id} className="p-5 bg-white border border-slate-100 rounded-3xl flex items-center justify-between group hover:border-blue-200 transition-all shadow-sm">
                          <div className="flex items-center space-x-4">
                            ...
                            <div>
                              ...
                              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{file.category} • {new Date(file.date).toLocaleDateString('pt-BR')}</p>
                            </div>
                          </div>
                          <div className="flex items-center space-x-2">
                      ...
                      <div className="col-span-2 py-12 text-center border-4 border-dashed border-slate-50 rounded-[40px] bg-slate-50/20">
                        <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Aguardando envio de exames ou laudos.</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 no-print">
          <div className="bg-white w-full max-w-4xl rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h2 className="text-2xl font-black text-slate-800 tracking-tight">{showEditModal ? 'Editar Registro Clínico' : 'Novo Paciente'}</h2>
              <button onClick={() => { setShowAddModal(false); setShowEditModal(false); }} className="p-3 text-slate-400 hover:bg-slate-200 rounded-2xl"><X size={24} /></button>
            </div>
            ...
            <form onSubmit={handleSavePatient} className="flex-1 overflow-y-auto p-10 space-y-10">
              ...
              <div className="space-y-6 pt-10 border-t border-slate-100">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Alergias Conhecidas (Separe por vírgula)</label>
                    <textarea value={formData.allergies} onChange={e => setFormData({...formData, allergies: e.target.value})} className="w-full p-4 bg-red-50/50 border border-red-100 rounded-2xl text-sm font-bold outline-none" rows={2} />
                    
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Histórico Resumido (Dica: Use [DATA: dd/mm/aaaa] para organizar)</label>
                    <textarea value={formData.history} onChange={e => setFormData({...formData, history: e.target.value})} className="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium leading-relaxed outline-none" placeholder="Descreva aqui o histórico detalhado..." rows={12} />
                  </div>
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Patologias Pré-existentes</label>
                    <textarea value={formData.preExistingConditions} onChange={e => setFormData({...formData, preExistingConditions: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none" rows={2} />
                  </div>
                </div>
              </div>

              <div className="pt-6 flex justify-end space-x-4 border-t border-slate-100">
                <button type="button" onClick={() => { setShowAddModal(false); setShowEditModal(false); }} className="px-8 py-4 text-slate-400 font-black text-xs uppercase tracking-widest">Descartar</button>
                <button type="submit" disabled={isSaving} className="px-12 py-4 bg-blue-600 text-white rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center space-x-2 transition-all hover:bg-blue-700 active:scale-95">
                  {isSaving ? <RefreshCw className="animate-spin" size={18} /> : <Save size={18} />}
                  <span>{showEditModal ? 'ATUALIZAR DADOS' : 'SALVAR PACIENTE'}</span>
                </button>
              </div>
            </form>
          </div>
      ...
      {viewingFile && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-slate-900/95 backdrop-blur-xl p-6 no-print">
          <div className="bg-white w-full max-w-6xl h-full max-h-[90vh] rounded-[40px] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            ...
            <div className="flex-1 bg-slate-100 overflow-auto flex items-center justify-center p-8">
              {viewingFile.type.includes('image') ? <img src={viewingFile.url} className="max-w-full max-h-full object-contain rounded-xl shadow-2xl" /> : viewingFile.type.includes('pdf') ? <iframe src={viewingFile.url} className="w-full h-full rounded-xl" title="Visualizador" /> : <div className="text-center p-12 bg-white rounded-3xl shadow-sm"><FileIcon size={48} className="mx-auto text-slate-200 mb-4" /><p className="font-bold text-slate-500 text-sm">Extensão de arquivo não suportada para visualização direta.</p></div>}
            </div>
          </div>
        </div>
      )}
...
const FormInput: React.FC<{ label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean }> = ({ label, value, onChange, type = 'text', required }) => (
  <div className="space-y-1.5">
    ...
    <input type={type} value={value} onChange={e => onChange(e.target.value)} required={required} className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm font-bold shadow-sm transition-all" />
  </div>
);
